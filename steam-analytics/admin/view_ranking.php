...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; font-family:'Poppins', sans-serif; }
        ...
        th, td {
            padding:16px; font-size:14px; color:#ffffff; border-top:1px solid rgba(255,255,255,0.05);
        }
        th { text-transform:uppercase; letter-spacing:1px; font-weight:500; text-align:left; }
        ...
        td.popularity { text-align:right; font-weight:600; padding-right:20px; }
        ...
        if($result->num_rows > 0){
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                        ...
                        <td>{$row['name']}</td>
                        <td class='popularity'>" . number_format($row['popularity_score']) . "</td>
                      </tr>";
                $rank++;
            }
        } else {
            echo "<tr><td colspan='3' class='empty'>No games found</td></tr>";
        }
        ?>
        </tbody>
    </table>
